					...
					<el-table-column align="right" prop="vedioUrlList">
						...
						<template slot-scope="scope">
							<el-button
								...
								type="success"
							>
								<el-link :href="bItem.url" target="_blank" rel="noopener noreferrer" download="1.mp4">{{bItem.definition}}</el-link>
							</el-button>
...
export default {
	...
	methods: {
		getCommitHistory() {
			this.$http.get("/xigua/commitHistory").then(res => {
				console.log(res);
		...
		getVedioPlayUrl(formInline) {
			this.loading = true;
			this.$http
				.post("/xigua/downloadUrl", { url: this.formInline.vedioUrl })
				.then(res => {
					let data = res.data;
					if (data.code != 200) {
						this.tip(data.message, "error");
					} else {
						this.tip();
